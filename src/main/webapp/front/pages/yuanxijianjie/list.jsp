      ...
      <form class="layui-form">
        <div class="layui-inline" style="margin-bottom: 10px;">
          <div class="layui-input-inline">
            <input type="text" name="yuanximingcheng" id="yuanximingcheng" placeholder="院系名称" autocomplete="off" class="layui-input">
          </div>
        </div>   
        <div class="layui-inline" style="margin-left: 30px;margin-bottom: 10px;">
          ...
          <button v-if="isAuth('yuanxijianjie','新增')" @click="jump('../yuanxijianjie/add.jsp')" type="button" class="layui-btn btn-theme">
                  ...
                  <div class="gallery-item isotope-item print-media col-lg-3 col-sm-3 col-xs-6" v-for="(item,index) in dataList" v-bind:key="index">
                    <div class="inner-contents">
                      <figure style="text-align: center;">
                        <a class="gallery-btn swipebox" :href="'../yuanxijianjie/detail.jsp?id='+item.id">查看更多</a>
                        <div class="media-container"></div>
                        <a :href="'../yuanxijianjie/detail.jsp?id='+item.id">
                          <img :src="item.tupian?item.tupian.split(',')[0]:''" width="210" height="150" />
                      </figure>
                      <h5 class="item-title"><a :href="'../yuanxijianjie/detail.jsp?id='+item.id">{{item.yuanximingcheng}}</a></h5>
  ...
  <script>
    ...
    layui.use(['layer', 'element', 'carousel', 'laypage', 'http', 'jquery'], function() {
      ...
      function pageList() {
        ...
        if (jquery('#yuanximingcheng').val()) {
          param['yuanximingcheng'] = jquery('#yuanximingcheng').val() ? '%' + jquery('#yuanximingcheng').val() + '%' : '';
        }  
        // 获取列表数据
        http.request('yuanxijianjie/list', 'get', param, function(res) {
